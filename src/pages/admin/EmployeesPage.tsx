...
// In dev: Vite proxies /api/pos/employees → pos.hilotcenter.cloud (bypasses CORS)
// In prod: requests go through the Supabase Edge Function
...
export function EmployeesPage() {
  ...
  // Fetch POS employees — always fresh
  ...
  // Fetch cooperative profiles (only those with employee_id, plus all for linking)
    ...
    queryFn: async (): Promise<CoopProfile[]> => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          full_name,
          employee_id,
          membership_status ( status, completed_shares )
        `)
  ...
  // Build a lookup map: employee_id → profile
  ...
  // Unlinked profiles (for the link dropdown)
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            All employees from the POS system. Employees can join the cooperative by purchasing at least 1 share.
          </p>
        </div>
        <button
          onClick={() => refetch()}
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <Button variant="secondary" onClick={() => { setLinkModal(null); setLinkError(null) }}>
                Cancel
              </Button>
              <Button
                ...
                onClick={() => linkEmployee.mutate({
                  ...
                  employeeId: linkModal.employee.employee_id,
                })}
              >
